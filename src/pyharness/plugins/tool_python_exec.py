...
from pyharness.schema import ToolArg, ToolResult, ToolResultStatus, ToolSpec, ToolStreamEvent
...
async def _read_proc_output(
    proc: "asyncio.subprocess.Process",
    timeout: float,
    tool_name: str,
    emitter: "Callable[[ToolStreamEvent], Awaitable[None]] | None",
) -> tuple[str, str]:
    """Stream a subprocess's stdout/stderr line-by-line to ``emitter``.

    Returns the complete collected ``(stdout, stderr)`` text once the process
    exits. Raises :class:`asyncio.TimeoutError` if the read exceeds ``timeout``;
    the caller is responsible for killing the process in that case.
    """

    async def _read_one(stream: Any, stream_type: str) -> str:
        if stream is None:
            return ""
        parts: list[str] = []
        async for raw in stream:
            ...
            if emitter is not None:
                try:
                    await emitter(
                        ToolStreamEvent(
                            tool_name=tool_name, stream_type=stream_type, content=text
                        )
                    )
                except Exception:
                    logger.warning("tool_emitter raised; suppressing", exc_info=True)
        return "".join(parts)

    stdout_text, stderr_text = await asyncio.wait_for(
        asyncio.gather(
            _read_one(proc.stdout, "stdout"),
            _read_one(proc.stderr, "stderr"),
        ),
        timeout=timeout,
    )
    # Reap the process (streams already hit EOF) so returncode is reliable.
    await proc.wait()
    return stdout_text, stderr_text


async def _execute_in_docker(code: str, workdir: Path, context: SessionContext) -> ToolResult:
    ...
    emitter = context.tool_emitter if context is not None else None
    ...
    try:
        ...
        try:
            stdout_text, stderr_text = await _read_proc_output(
                proc, _EXEC_TIMEOUT, "python_exec", emitter
            )
...
async def _execute_in_subprocess(code: str, context: SessionContext) -> ToolResult:
    """Execute Python code in a local subprocess, streaming output."""
    emitter = context.tool_emitter if context is not None else None
    try:
        proc = await asyncio.create_subprocess_exec(
            sys.executable,
            "-u",  # unbuffered: ensure line-by-line streaming to emitter
...
class PythonExecPlugin:
    ...
    async def execute_tool(
        ...
        if mode == "subprocess":
            return await _execute_in_subprocess(code, context)
        ...
        if mode == "docker":
            ...
            return await _execute_in_docker(code, workdir, context)
