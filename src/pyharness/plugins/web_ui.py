...
from pyharness.schema import AgentConfig, ContentPart, Event, Message, Role, SessionData
...
def _decode_data_url(data_url: str) -> tuple[bytes, str]:
    ...
    mime = meta[len("data:") :].split(";")[0] or "application/octet-stream"
    return base64.b64decode(b64), mime


async def _prepare_user_message(content: str, parts_raw: list[dict[str, Any]]) -> Message:
    """Build a :class:`Message` from raw WebSocket parts.

    Audio parts are transcribed via :func:`transcribe_bytes` (batch 2) and the
    transcript is appended as a ``text`` part; transcription failures are logged
    but never block the send.
    """
    parts: list[ContentPart] = []
    for p in parts_raw or []:
        ptype = p.get("type")
        if ptype in ("image", "audio"):
            url = p.get("url")
            if url:
                parts.append(ContentPart(type=ptype, url=url))  # type: ignore[arg-type]
        elif ptype == "text":
            parts.append(ContentPart(type="text", text=p.get("text")))  # type: ignore[arg-type]

    for p in list(parts):
        if p.type == "audio" and p.url:
            try:
                data, mime = _decode_data_url(p.url)
                transcript = await transcribe_bytes(data, mime)
                parts.append(
                    ContentPart(type="text", text=f"[音频转写]: {transcript}")  # type: ignore[arg-type]
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning("audio transcription failed (skipped): %s", exc)

    return Message(role=Role.USER, content=content or "", parts=tuple(parts))
...
class WebUIPlugin:
    ...
    async def _websocket_events(self, ws: WebSocket) -> None:
        ...
        try:
            while True:
                # Defensive receive: a payload that exceeds the WebSocket frame
                # size limit (or any transport error) must not crash the socket.
                try:
                    raw = await ws.receive_text()
                except WebSocketDisconnect:
                    raise
                except Exception as exc:
                    logger.warning("WebSocket receive failed (possible oversized payload): %s", exc)
                    try:
                        await self._send(
                            ws,
                            "error",
                            {"message": "消息接收失败，可能超过大小限制，请减小附件体积后重试。"},
                        )
                    except Exception:
                        pass
                    break
                ...
                try:
                    ...
                    logger.warning("WebSocket JSON parse failed: %s", exc)
                    await self._send(ws, "error", {"message": "消息格式错误，无法解析。"})
                    continue
                ...
                if msg_type == "user_message":
                    ...
                    current_task = asyncio.create_task(
                        self._run_stream_session(
                            session_id, content, model=model, ws=ws, parts_raw=parts_raw
                        )
                    )
    ...
    async def _run_stream_session(
        ...
        model: str | None = None,
        ws: Any = None,
        parts_raw: list[dict[str, Any]] | None = None,
    ) -> None:
        ...
        try:
            user_message = await _prepare_user_message(content or "", parts_raw or [])
...
def serve(harness: Any, host: str = "127.0.0.1", port: int = 3080) -> None:
    ...
    harness.initialize()
    plugin.mount_static()
    # ws_max_size: allow large multimodal Base64 payloads (image/audio) over the
    # WebSocket without hitting Uvicorn's default 16MB frame limit (and 1009
    # "Message too big"). 32MB gives comfortable headroom for ~4MB files that
    # inflate ~33% after Base64 plus JSON wrapping.
    uvicorn.run(plugin.app, host=host, port=port, ws_max_size=32 * 1024 * 1024)
