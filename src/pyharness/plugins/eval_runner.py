...
from pyharness.schema import AgentConfig, Event, LLMRequest, LLMResponse, Message, Role
    ...
    async def run_task(self, task: EvalTask) -> EvalResult:
        ...
        try:
            ...
            ctx = await asyncio.wait_for(
                self.harness.run_session(
                    agent=agent,
                    initial_text=task.prompt,
                ),
...
async def _settle(values: tuple[Any, ...]) -> list[Any]:
    """Await any coroutines returned by async hookimpls."""
    import inspect
    ...
    for value in values:
        try:
            ...
            logger.debug("_settle ignored exception: %s", exc)
    return settled
